---
name: vault-resume
description: Resume work from the last saved session checkpoint. Use when continuing an in-progress vault session and carrying forward only the relevant handoff context.
---
...
Accepts an optional `--session <session-id>` argument. If no arguments are provided, locate the most recent session file in `.agent-vault/05_Sessions/` and resume from its last recorded state.
...
- If no sessions exist, report that there is nothing to resume and suggest using the vault-execute workflow instead.
...
- This new session is now the active session for the entire conversation. Do not create additional sessions unless the user explicitly requests one.

### 5. Transition into execution.
- Run the readiness-checklist preflight from the vault-execute workflow (step 3) against the target step.
- If the step passes readiness, proceed with implementation using the vault-execute workflow steps 5 through 7 (research, implement-validate-iterate, close the loop).
...
- When work pauses or the conversation ends, the session's last Execution Log entry and Follow-Up Work section serve as the handoff for the next resume.
