...
/**
 * Path to the original vault templates in the fred repo.
 * Tests copy templates from here to temp vaults.
 */
const FRED_VAULT_ROOT = join(dirname(fileURLToPath(import.meta.url)), '..', '..', 'fred', '.agent-vault');

export const copyTemplate = async (tempVaultRoot: string, templateName: string): Promise<void> => {
  const source = join(FRED_VAULT_ROOT, '07_Templates', templateName);
...
export const copyHomeNote = async (tempVaultRoot: string, noteName: string): Promise<void> => {
  const source = join(FRED_VAULT_ROOT, '00_Home', noteName);
