...
const COMMANDS: readonly AgentVaultCommandDefinition[] = [
  ...
  },
  {
    ...
    notes: [
      ...
      '`--apply` runs pending registry steps strictly in order, validates the vault after each step before advancing `vault_schema_version`, stops and reports at unsafe/manual steps, and refreshes the code graph after any run that advanced the version.',
      '`--apply --to <version>` stops after reaching the named intermediate version; `--to` requires `--apply`, must match a registered step boundary, and refuses downgrades or versions past the package latest.',
      'Interrupted applies are resumable: the schema version only advances after a step\'s writes and post-step validation succeed, so re-running `vault migrate --apply` resumes from the incomplete step.',
