...
import { applyThinStepNotesMigration } from './migrations/steps/0001-thin-step-notes';
import {
...
export async function handleMigrateStepNotesCommand(
  ...
  try {
    ...
    const result = await applyThinStepNotesMigration(
      { vaultRoot },
      { phaseRef, stepRef, now: environment.now },
    );
    ...
    io.stdout(`Migrated ${result.migratedCount} legacy step note${result.migratedCount === 1 ? '' : 's'}.`);
